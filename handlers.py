...
def check_spam(uid):
    ...
    if len(user_actions[uid]) > 5:
        ...
        try: bot.send_message(uid, "🛡 **ANTI-SPAM TRIGGERED!**\nYou are temporarily blocked for 5 minutes.", parse_mode="Markdown")
        except: pass
        return True
    return False
...
# ==========================================
# ২. UI ENGINE & CYBERPUNK ANIMATION HELPER
# ==========================================
def play_loading(chat_id, t1, t2, t3):
    """Universal function to play cool loading animations"""
...
def sub_check_callback(call):
    ...
    if check_sub(call.message.chat.id):
        ...
        bot.send_message(call.message.chat.id, "✅ **Access Granted! Welcome to the Elite Club.**", reply_markup=main_menu(), parse_mode="Markdown")
    else: 
...
# ==========================================
# ৪. ORDERING SYSTEM & SMART CATEGORIES
# ==========================================
...
def show_platforms(message):
    ...
    if not services: return bot.edit_message_text("❌ **API Offline.**", message.chat.id, msg_id, parse_mode="Markdown")
    ...
    bot.edit_message_text("🟢 **Live API Status:** Active\n━━━━━━━━━━━━━━━━━━━━\n📂 **Select a Platform:**", message.chat.id, msg_id, reply_markup=markup, parse_mode="Markdown")
...
def list_services(call):
    ...
    for s in filtered[start:end]:
        ...
        speed_emoji = "🚀" if "instant" in s['name'].lower() or "fast" in s['name'].lower() else "💧" if "drip" in s['name'].lower() else "🐢" if "slow" in s['name'].lower() else "⚡"
    ...
    markup.add(types.InlineKeyboardButton(f"🔙 Back", callback_data=f"PLAT|{platform_name}|0"))
    bot.edit_message_text(f"📍 **{platform_name}**\n📦 **{cat_name[:30]}...**\nPage: {page+1}", call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode="Markdown")
...
def order_preview(message, sid, link):
    try:
        ...
        if not s or qty < int(s['min']) or qty > int(s['max']): return bot.send_message(message.chat.id, f"❌ Invalid Quantity! Min: {s['min']}")

        user = users_col.find_one({"_id": message.chat.id})
        _, discount = get_user_tier(user.get('spent', 0))
        global_profit = get_settings().get("profit_margin", 20.0)
        cost = round(((float(s['rate']) * (1 + global_profit/100)) * (1 - discount/100) / 1000) * qty, 3)
        
        if user['balance'] < cost: return bot.send_message(message.chat.id, f"❌ **Insufficient Balance!**\nNeed `${cost}`, you have `${round(user['balance'],3)}`.")
...
def deposit_ask_amount(message):
    try:
        ...
        if not payments: return bot.send_message(message.chat.id, "❌ No payment methods active.")
        
        txt = f"💰 **DEPOSIT: ${amount}**\n━━━━━━━━━━━━━━━━━━━━\nSelect a Gateway:"
        markup = types.InlineKeyboardMarkup(row_width=1)
        for p in payments:
            markup.add(types.InlineKeyboardButton(f"🏦 {p['name']} ({round(amount * float(p['rate']), 2)} BDT)", callback_data=f"PAY|{amount}|{p['name']}"))
...
def deposit_show_details(call):
    ...
    msg_id = play_loading(call.message.chat.id, "Securing Connection", "Fetching Payment Details", "Generating Invoice")
    
    bdt_amount = round(amt * float(method['rate']), 2)
    txt = f"🏦 **{method_name} DEPOSIT**\n━━━━━━━━━━━━━━━━━━━━\n💵 **Amount to Pay:** `{bdt_amount} BDT`\n👉 **Send to:** `{method['details']}`\n━━━━━━━━━━━━━━━━━━━━\nReply with your **TrxID** or Screenshot Link."
    msg = bot.edit_message_text(txt, call.message.chat.id, msg_id, parse_mode="Markdown")
...
# ==========================================
# ৬. MENU BUTTONS & UNIVERSAL ACTIONS
# ==========================================
...
def convert_currency(call):
    ...
    if curr == "BDT": bot.answer_callback_query(call.id, f"৳ Your Wallet: {round(bal * 120, 2)} BDT", show_alert=True)
    elif curr == "INR": bot.answer_callback_query(call.id, f"₹ Your Wallet: {round(bal * 83, 2)} INR", show_alert=True)
...
def universal_buttons(message):
    ...
    if message.text == "👤 Profile":
        msg_id = play_loading(message.chat.id, "Accessing Vault", "Fetching Statistics", "Generating VIP Card")
        ...
        card = (f"```text\n╔════════════════════════════════╗\n║  🌟 NEXUS TITAN VIP CARD       ║\n╠════════════════════════════════╣\n║  👤 Name: {name_str.ljust(19)}║\n║  🆔 UID: {uid_str.ljust(20)}║\n║  💳 Balance: {bal_str.ljust(16)}║\n║  💸 Spent: {spent_str.ljust(18)}║\n║  👑 Tier: {tier.ljust(19)}║\n╚════════════════════════════════╝\n```")
        markup = types.InlineKeyboardMarkup(row_width=2).add(types.InlineKeyboardButton("🇧🇩 BDT", callback_data="CURR|BDT"), types.InlineKeyboardButton("🇮🇳 INR", callback_data="CURR|INR"))
        ...
        msg_id = play_loading(message.chat.id, "Calculating Links", "Fetching Referrals", "Loading Dashboard")
        u = users_col.find_one({"_id": message.chat.id})
        ...
        msg_id = play_loading(message.chat.id, "Scanning Database", "Ranking Users", "Generating Leaderboard")
        ...
        bot.register_next_step_handler(msg, lambda m: bot.send_message(m.chat.id, "Processing...") and process_smart_search(m))
        
    elif message.text == "🎧 Support Ticket":
        msg = bot.send_message(message.chat.id, "🎧 **Describe your issue clearly:**", parse_mode="Markdown")
        bot.register_next_step_handler(msg, lambda m: [tickets_col.insert_one({"uid": m.chat.id, "msg": m.text, "status": "open"}), bot.send_message(m.chat.id, "✅ Ticket Sent!", parse_mode="Markdown")])
...
def ai_smart_assistant(message):
    ...
    if any(word in text for word in ["buy", "need", "want", "views", "likes", "followers"]):
        services = get_cached_services()
        hidden = get_settings().get("hidden_services", [])
        keywords = [w for w in text.split() if w not in ["buy", "need", "want", "some", "for"]]
        results = [s for s in services if str(s['service']) not in hidden and any(k in s['name'].lower() for k in keywords)][:5]
        if results:
            markup = types.InlineKeyboardMarkup(row_width=1)
    ...
    try:
        ...
        markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🗣 Transfer to Live Admin", callback_data="TALK_HUMAN"))
