...
def start(message):
    ...
    # 🔥 FEATURE: Cyberpunk Welcome Message
...
def info_card(call):
    ...
    # 🔥 FEATURE: Live Average Time logic
    avg_time = s.get('time', 'Instant - 24h')
    if not avg_time or avg_time == "N/A": avg_time = "Instant - 24h"
...
def text_router(message):
    ...
    # 🔥 FEATURE: Admin Chat Reply Logic (Direct from Telegram)
    ...
    if step == "awaiting_link":
        ...
        if existing:
            bot.send_message(uid, "⚠️ **WARNING: Order Already Pending!**\nYou already have an active order with this link. Are you sure you want to proceed?", parse_mode="Markdown")
        ...
        # 🔥 FEATURE: Ticket alert directly to Admin for Quick Reply
        msg_text = f"🎧 **NEW SUPPORT TICKET**\n👤 ID: `{uid}`\n📝 Msg: {text}\n\n_Reply to this message directly to answer the user._"
