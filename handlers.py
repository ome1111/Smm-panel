...
# ASCII Encoding Fix (Bangla Support for Render)
...
def update_spy(uid, action_text):
    ...
    try: 
        users_col.update_one(
            {"_id": uid}, 
            {"$set": {"last_action": action_text, "last_active": now}}
        )
...
def check_spam(uid):
    ...
    if uid in blocked_users:
        if current_time < blocked_users[uid]: 
            return True
        else: 
    ...
    if len(user_actions[uid]) > 5:
        ...
        try: 
            bot.send_message(uid, "🛡 **ANTI-SPAM!** You are temporarily blocked for 5 minutes.", parse_mode="Markdown")
        except Exception: 
            pass
        return True
...
def check_maintenance(chat_id):
    if get_settings().get('maintenance', False) and str(chat_id) != str(ADMIN_ID):
        bot.send_message(
            chat_id, 
            "🛠 **SYSTEM UPDATE**\nBot is currently upgrading to serve you better. Please try again later.", 
            parse_mode="Markdown"
        )
        return True
    return False

# 🔥 UNIQUE LOADING EFFECTS ENGINE
...
# ==========================================
# 3. UI LOGIC & SMART SERVICE CLEANER
# ==========================================
...
def ghost_mode(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "⚠️ Format: `/ghost UserID`")

# ==========================================
# 5. REGISTRATION & AUTO-GREETING
# ==========================================
@bot.message_handler(commands=['start'])
def start(message):
    update_spy(message.chat.id, "Bot Started")
    
    if check_spam(message.chat.id) or check_maintenance(message.chat.id): 
        return
    ...
    if not check_sub(uid):
        ...
        return bot.send_message(
            uid, 
            "🛑 **ACCESS RESTRICTED**\nJoin our official channels to unlock features.", 
            reply_markup=markup, 
            parse_mode="Markdown"
        )
...
# ==========================================
# 6. FAST ORDERING & PLATFORM SELECTION
# ==========================================
...
def new_order_start(message):
    ...
    if check_spam(message.chat.id) or check_maintenance(message.chat.id) or not check_sub(message.chat.id): 
        return
    
    msg = bot.send_message(message.chat.id, "🌍 Connecting...")
    show_loading(message.chat.id, msg.message_id, ["🌍 Connecting...", "🌎 Fetching Data...", "🌏 Syncing API...", "✅ Ready!"])
    
    services = get_cached_services()
    if not services: 
        return bot.edit_message_text(
            "⏳ **API Syncing...** Please try again in 1 minute.", 
            message.chat.id, 
            msg.message_id, 
            parse_mode="Markdown"
        )
...
def show_cats(call):
    ...
    nav = []
    if page > 0: 
        nav.append(types.InlineKeyboardButton("⬅️ Prev", callback_data=f"PLAT|{platform_name}|{page-1}"))
    if end_idx < len(all_cats): 
        nav.append(types.InlineKeyboardButton("Next ➡️", callback_data=f"PLAT|{platform_name}|{page+1}"))
        
    if nav: 
        markup.row(*nav)
        
    bot.edit_message_text(
        f"📍 **Home** ➔ **{platform_name}**\n━━━━━━━━━━━━━━━━━━━━\n📂 **Choose Category:**", 
...
def list_servs(call):
    ...
    for s in filtered[start_idx:end_idx]:
        ...
        markup.add(types.InlineKeyboardButton(
            f"ID:{s['service']} | {fmt_curr(rate_usd, curr)} | {clean_service_name(s['name'])}", 
            callback_data=f"INFO|{s['service']}"
        ))
        
    nav = []
    if page > 0: 
        nav.append(types.InlineKeyboardButton("⬅️ Prev", callback_data=f"CAT|{cat_idx}|{page-1}"))
    if end_idx < len(filtered): 
        nav.append(types.InlineKeyboardButton("Next ➡️", callback_data=f"CAT|{cat_idx}|{page+1}"))
    ...
    bot.edit_message_text(
        f"📍 **{identify_platform(cat_name)}** ➔ **{cat_name[:20]}**\n━━━━━━━━━━━━━━━━━━━━\n📦 **Select Service:**", 
        call.message.chat.id, 
        call.message.message_id, 
        reply_markup=markup, 
        parse_mode="Markdown"
...
def info_card(call):
    ...
    if not s: 
        return bot.send_message(call.message.chat.id, "❌ Service currently unavailable.")
    ...
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("🛒 Order Now", callback_data=f"ORD|{sid}"), 
        types.InlineKeyboardButton("⭐ Fav", callback_data=f"FAV_ADD|{sid}")
    )
    
    try: 
        cat_idx = sorted(list(set(x['category'] for x in services))).index(s['category'])
    except Exception: 
        cat_idx = 0
...
def confirm_ord(message, sid, link):
    try:
        ...
        if user['balance'] < cost_usd:
            return bot.send_message(
                message.chat.id, 
                f"❌ **Insufficient Balance!** Need `{fmt_curr(cost_usd, curr)}`.", 
                parse_mode="Markdown"
            )

        users_col.update_one(
            {"_id": message.chat.id}, 
            {"$set": {"draft": {"sid": sid, "link": link, "qty": qty, "cost": cost_usd}}}
        )
        ...
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(
            types.InlineKeyboardButton("✅ CONFIRM", callback_data="PLACE_ORD"), 
            types.InlineKeyboardButton("❌ CANCEL", callback_data="CANCEL_ORD")
        )
...
# 🔥 REAL ORDER PLACED & FORWARDED TO PROOF CHANNEL (CYBER BOX)
...
def final_ord(call):
    ...
    if not draft or user['balance'] < draft['cost']: 
        return bot.send_message(call.message.chat.id, "❌ Session expired or low balance.")
    ...
    show_loading(
        call.message.chat.id, 
        call.message.message_id, 
        ["🛒 Preparing...", "🛒📦 Processing...", "🛒📦📦 Sending to API...", "✅ Order Placed!"]
    )
...
_Thank you for choosing Nexus SMM!_"""
        ...
        # 2. 🔥 REAL ORDER PROOF FORWARD TO PUBLIC CHANNEL (Cyber Box Style)
        s = get_settings()
        ...
        if proof_ch:
            try:
                services = get_cached_services()
                s_data = next((x for x in services if str(x['service']) == str(draft['sid'])), None)
                
                if s_data:
                    s_name = clean_service_name(s_data['name'])[:18] + ".." 
                else:
                    s_name = "Premium Service"
                
                proof_text = f"> ╔════ 🟢 𝗡𝗘𝗪 𝗢𝗥𝗗𝗘𝗥 ════╗\n> ║ 👤 𝗜𝗗: `***{str(call.message.chat.id)[-4:]}`\n> ║ 🚀 𝗦𝗲𝗿𝘃𝗶𝗰𝗲: {s_name}\n> ║ 📦 𝗤𝘁𝘆: {draft['qty']}\n> ║ 💵 𝗖𝗼𝘀𝘁: `${round(draft['cost'], 2)}`\n> ╚════════════════════╝"
                bot.send_message(proof_ch, proof_text, parse_mode="Markdown")
            except Exception:
                pass
    else:
        bot.edit_message_text(
            f"❌ **Error:** {res.get('error', 'API Timeout')}", 
            call.message.chat.id, 
            call.message.message_id, 
            parse_mode="Markdown"
        )
...
# ==========================================
# 7. PROFILE, ORDERS & QUICK REORDER
# ==========================================
...
def profile(message):
    ...
    show_loading(
        message.chat.id, 
        msg.message_id, 
        ["⭐ Loading Profile.", "🌟 Loading Profile..", "✨ Loading Profile...", "✅ Welcome!"]
    )
...
def set_curr(call):
    ...
    bot.edit_message_text(
        f"✅ **App Currency updated to {new_curr}!**", 
        call.message.chat.id, 
        call.message.message_id, 
        parse_mode="Markdown"
    )
...
def fetch_orders_page(chat_id, page=0):
    ...
    for o in page_orders:
        ...
        if current_status == 'completed':
            status_text = f"✅ {current_status.upper()}"
        elif current_status in ['canceled', 'refunded', 'fail', 'error']:
            status_text = f"❌ {current_status.upper()}"
        else:
            status_text = f"⏳ {current_status.upper()}"
    ...
    nav = []
    if page > 0: 
        nav.append(types.InlineKeyboardButton("⬅️ Prev", callback_data=f"MYORD|{page-1}"))
    if end < len(all_orders): 
        nav.append(types.InlineKeyboardButton("Next ➡️", callback_data=f"MYORD|{page+1}"))
...
# ==========================================
# 8. FULL UNIVERSAL SYSTEM (SEARCH, VOUCHER, DEPOSIT)
# ==========================================
...
def universal_buttons(message):
    ...
    if message.text == "📦 Orders":
        ...
        txt = "🏆 **TOP 5 SPENDERS**\n━━━━━━━━━━━━━━━━━━━━\n"
        for i, t_u in enumerate(top): 
            txt += f"{i+1}. {t_u['name']} - `{fmt_curr(t_u.get('spent',0), curr)}`\n"
        bot.send_message(message.chat.id, txt, parse_mode="Markdown")
        ...
        bot.register_next_step_handler(
            msg, 
            lambda m: [
                tickets_col.insert_one({"uid": m.chat.id, "msg": m.text, "status": "open", "date": datetime.now()}), 
                bot.send_message(m.chat.id, "✅ Ticket Sent Successfully!")
            ]
        )
        ...
        if not favs: 
            return bot.send_message(message.chat.id, "📭 You have no favorites.")
        ...
        for sid in favs:
            s = next((x for x in services if str(x['service']) == str(sid)), None)
            if s: 
                markup.add(types.InlineKeyboardButton(f"⭐ ID:{s['service']} | {clean_service_name(s['name'])[:25]}", callback_data=f"INFO|{s['service']}"))
...
def process_voucher(message):
    ...
    if not voucher: 
        return bot.send_message(message.chat.id, "❌ Invalid Voucher Code.")
        
    if len(voucher.get('used_by', [])) >= voucher['limit']: 
        return bot.send_message(message.chat.id, "❌ Voucher Limit Reached!")
        
    if message.chat.id in voucher.get('used_by', []): 
        return bot.send_message(message.chat.id, "❌ You have already claimed this voucher.")
    ...
    bot.send_message(
        message.chat.id, 
        f"✅ **VOUCHER CLAIMED**\nReward: `{fmt_curr(voucher['amount'], curr)}` added to your wallet.", 
        parse_mode="Markdown"
    )
...
def process_smart_search(message):
    ...
    show_loading(
        message.chat.id, 
        msg.message_id, 
        ["📡 Scanning Database.", "📡 Scanning Database..", "📡 Scanning Database...", "✅ Searching Completed!"]
    )
    ...
    if query.isdigit():
        ...
        if s: 
            ...
            return bot.edit_message_text(
                f"✅ **Found:** {clean_service_name(s['name'])}", 
                message.chat.id, 
                msg.message_id, 
                reply_markup=markup, 
                parse_mode="Markdown"
    ...
    if not results: 
        return bot.edit_message_text("❌ No related services found.", message.chat.id, msg.message_id)
...
def pay_details(call):
    ...
    bot.edit_message_text(
        f"🏦 **{method} Payment**\nSend money and reply with **TrxID/Transaction ID**.", 
        call.message.chat.id, 
        call.message.message_id
    )
...
def process_deposit_trx(message, amt, method_name):
    ...
    show_loading(
        message.chat.id, 
        msg.message_id, 
        ["🪙 Initializing...", "💰 Verifying Payment...", "💳 Securing TrxID...", "✅ Wallet Updated!"]
    )
    
    bot.edit_message_text(
        "✅ **Request Submitted!**\nAdmin will verify your TrxID shortly.", 
        message.chat.id, 
        msg.message_id, 
        parse_mode="Markdown"
    )
...
def ask_refill(call):
    ...
    bot.register_next_step_handler(
        msg, 
        lambda m: [
            bot.send_message(m.chat.id, "✅ Refill Requested!"), 
            bot.send_message(ADMIN_ID, f"🔄 **REFILL:** Order `{m.text}` by `{m.chat.id}`")
        ]
    )
...
def talk_to_human(call):
    ...
    bot.register_next_step_handler(
        ...
        lambda m: [
            tickets_col.insert_one({"uid": m.chat.id, "msg": m.text, "status": "open", "date": datetime.now()}), 
            bot.send_message(m.chat.id, "✅ Ticket Sent!")
        ]
    )
...
def ai_chat(message):
    ...
    show_loading(
        message.chat.id, 
        msg.message_id, 
        ["🧠 AI Thinking.", "🧠 AI Thinking..", "🧠 AI Thinking...", "🤖 Nexus AI Replying!"]
    )
